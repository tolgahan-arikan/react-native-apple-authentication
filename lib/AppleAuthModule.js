...
export default class AppleAuthModule {
  ...
  performRequest(
    ...
    if (!Object.hasOwnProperty.call(requestOptions, ['hashNonceAutomatically'])) {
      requestOptions.hashNonceAutomatically = true;
    } else if (typeof requestOptions.nonceEnabled !== 'boolean') {
      throw new Error(
        "RNAppleAuth.AppleAuthRequestOptions 'nonceEnabled' must be a boolean value if provided.",
      );
    ...
    const { requestedScopes, requestedOperation, user, nonce, nonceEnabled, hashNonceAutomatically, state, ...rest } = requestOptions;
    ...
    console.log('hashNonceAutomatically', hashNonceAutomatically);

    return this.native.performRequest(requestOptions, hashNonceAutomatically);
  }
